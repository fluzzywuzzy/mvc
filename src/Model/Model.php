...
abstract class Model implements Model_Interface {
	...
	static public function upsert($data, $unique_keys = null, $dont_update_keys = null) {
		...
		$db = self::db();
		$data = static::alter_data($data);
		$result = $db->upsert(static::get_table(), $data, $unique_keys, $dont_update_keys, static::IS_AUTO_INCREMENT ? static::PRIMARY_KEY : null);
	...
	static public function collection(): Collection_Interface {
		...
		foreach(class_parents(get_called_class()) as $parent_class) {
			...
			if(class_exists($collection_class_name) && (new \ReflectionClass($collection_class_name))->isInstantiable()) {
				return new $collection_class_name;
			}
		}
